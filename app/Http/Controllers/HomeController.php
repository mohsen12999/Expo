...
class HomeController extends Controller
{
    ...
    public function expo($id)
    {
        ...
        $map_ids = Booth::where('expo_id', $id)->select('booth_map_id')->get()->toArray();

        return view('home.expo', compact('booths', 'comments', 'images', 'expo', 'menu', 'map_ids'));
    }
    ...
    {
        $menu = '';
        $expos = Expo::where([['status', 1], ['end', '<', Carbon::today()->toDateString()], ['history', 1]])->get();
