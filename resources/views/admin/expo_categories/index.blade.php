...
@section('title',  __('words.ExpoCategoryList'))
...
@section('content')
<section>
    <h1>{{ __('words.ExpoCategoryList') }}</h1>
</section>
...
<div class="col-xs-12">
  <div class="box">
    ...
    <div class="box-header">
      <h3 class="box-title">{{ __('words.ExpoCategoryList') }}</h3>
      <a href="{{url('admin\expo-category\create')}}" class="btn btn-primary pull-right" style="margin-left: 1em;"><i class="fa fa-plus"></i> {{ __('words.NewCategory') }}</a>
    </div>
    <!-- /.box-header -->
    ...
    <div class="box-body table-responsive no-padding">
      ...
      <table class="table table-hover persian-table table-striped table-bordered">
        ...
        <tr>
          ...
          <td>
            <a class="btn btn-warning" href="{{action('Admin\ExpoCategoryController@edit', $category['id'])}}">
              <i class="fa fa-edit" title="{{ __('words.ChangeCategory') }}"></i>{{ __('words.ChangeCategory') }}
            </a>
            <form style="display:inline;" action="{{ url('admin/expo-category/' . $category->id) }}" method="post"
