        ...
        <div class="expo-map col-12">
            ...
            @foreach ($booth_numbers as $booth_number)
                @if (in_array($booth_number,array_column($other_booths_array,'booth_map_id')))
                    ...
                    @endphp
                    <button type="button" disabled class="btn btn-secondary btn-{{$booth_number}}" title="{{$this_booth->user->name}}">{{$booth_number}}</button>
                @else
                    <button type="submit" formaction="/admin/booth/booth-location/{{$booth_number}}" class="btn btn-primary btn-{{$booth_number}}">{{$booth_number}}</button>
                @endif
            @endforeach
        </div>
